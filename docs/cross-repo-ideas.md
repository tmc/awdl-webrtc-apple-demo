...
| Add a Bonjour-discovered control path | Grove's Network.framework transport can advertise one service and dial a peer service by name; WeightShare uses `NWEndpointCreateBonjourService` for AWDL data transfers. | `answer-bonjour`/`offer-bonjour` now advertise and browse `_awdl-webrtc-signal._tcp` and carry the same WebRTC `OFFER`/`ANSWER` wire signal. Local DNS-SD browse/resolve works, but same-host Network.framework self-dialing remains in `NWConnectionStatePreparing`; two-host proof is still pending. |
...
- WeightShare's `dns-sd` parsing is pragmatic for app discovery. The demo
  ...
  TXT records and AWDL peer-to-peer parameters remain structured; `dns-sd` is
  still useful as an external diagnostic when local `NWConnection` dialing does
  not reach `Ready`.
...
6. Run Bonjour signaling on two Macs once the peer is reachable; the local
   same-host path advertises and resolves but does not reach Network.framework
   TCP `Ready`.
