...
| Avoid SSH as the only control plane | `offer-stdio`, `answer-stdio`, `offer-bonjour`, and `answer-bonjour`; README manual and Bonjour signaling workflows. | Parser tests pass; local stdio smoke exchanged `OFFER` and `ANSWER` lines. Bonjour signaling advertises `_awdl-webrtc-signal._tcp`; DNS-SD browse/resolve and BSD TCP reachability to the advertised host:port passed locally. | Same-host stdio datachannel did not open. Same-host Network.framework Bonjour dialing stays in `NWConnectionStatePreparing`; two-host Bonjour signaling still needs a reachable peer. |
