...
| SwiftUI text layout and harness | PASS for rendering bug fix and launch harness: the local UI active-path run no longer uses zero-height text-cell frames, shortens long endpoints, and showed a readable Thunderbolt active path fed by a remote headless discovery publisher. `scripts/run-ui-harness.sh` now makes the remote publisher plus local UI launch repeatable for the physical proof. `LAUNCH_UI=0 ... scripts/run-ui-harness.sh` passed from clean commit `d7da303`, matching the current remote publisher and seeing LAN, Thunderbolt, and AWDL ready. This is layout and harness evidence only, not physical fallback completion. |
