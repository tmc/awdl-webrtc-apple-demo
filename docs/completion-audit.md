...
| Gate | Current result |
| --- | --- |
| Local gates | PASS for the published-module demo gate on 2026-05-20: `test -x scripts/run-ui-harness.sh && bash -n scripts/run-ui-harness.sh`, `GOWORK=off go test ./...`, and `GOWORK=off go vet ./...` passed with `github.com/tmc/apple v0.6.7` and `github.com/tmc/apple-pion v0.1.3`. |
| Workspace modules | BLOCKED for the current local gate: checked-in `go.work` uses `.`, `../apple`, and `../apple-pion`, but `go test ./...` now fails in dirty sibling `../apple/foundation/nsuuid.gen.go` with `undefined: kernel.Uuid_t`. The 2026-05-19 matrix binary was built from the workspace and remains historical link evidence. See [workspace-gate-20260520.md](workspace-gate-20260520.md). |
...
| SwiftUI text layout and harness | PASS for rendering bug fix, selected-peer targeting, and visible active-path proof: the local UI active-path run no longer uses zero-height text-cell frames, shortens long endpoints, and showed a readable Thunderbolt active path fed by a remote headless discovery publisher. `scripts/run-ui-harness.sh` now makes the remote publisher plus local UI launch repeatable for the physical proof, passes the selected remote service into `-mode ui`, and preserves the checked-in workspace build by default while allowing `BUILD_GOWORK=off` for an explicit published-module UI proof. `LAUNCH_UI=0 ... scripts/run-ui-harness.sh` passed from clean commit `d7da303`, and `LAUNCH_UI=0 BUILD_GOWORK=off ... scripts/run-ui-harness.sh` passed from clean commit `1288332`, matching the current remote publisher and seeing LAN, Thunderbolt, and AWDL ready. A 2026-05-20 `BUILD_GOWORK=off` visible run captured `/tmp/awdl-webrtc-ui-rerun-focused-20260520.png` with active path `thunderbolt`, peer `MacBook-m4small.local`, 0.0% loss history, and all three possible paths. `/tmp/awdl-webrtc-ui-direct-probe-20260520.txt` then proved the same published-module build could send UDP over Thunderbolt, AWDL, and LAN to the remote publisher with 0.00% loss and path evidence. Physical fallback is still not complete because unprivileged `ifconfig bridge0 down` returned `permission denied`. |
...
| Keep backends pluggable | `-backend go\|network`; `-pion-net` switch from `SetICEUDPMux` to `SettingEngine.SetNet`; `github.com/tmc/apple-pion/nwtransport`; checked-in `go.work`. | Published-module `GOWORK=off` tests pass today; LAN, Thunderbolt, and AWDL `-pion-net` WebRTC pass in the workspace-built two-host matrix; `../apple-pion` is clean and its tests assert native numeric UDP ownership and fallback for named UDP addresses. | Broader all-Network.framework TCP/TURN/STUN ownership remains outside this demo. Current `go.work` test rerun is blocked by dirty `../apple`; see [workspace-gate-20260520.md](workspace-gate-20260520.md). |
| Prefer `go.work` over new tags | `go.work`; local sibling `../apple` and `../apple-pion`; no new tags. | The 2026-05-19 matrix binary was built from the workspace, and no tags were created after the direction change. | Publish later only when these local changes are ready. Current local `go.work` verification is blocked by dirty `../apple`. |
...
| Package reusable pieces out of the temporary demo | `github.com/tmc/apple/x/network/nwpacket`; `github.com/tmc/apple-pion/nwtransport`; `github.com/tmc/apple-pion/icepolicy`. | Reusable packages have tests/examples and are consumed by the demo through the checked-in workspace for the historical two-host proof. Current published-module demo tests pass. | The current checked-in workspace gate must be rerun after dirty `../apple` is clean. |
| Add SwiftUI link-health view with fallback | `ui.go`; `ui_test.go`; `scripts/run-ui-harness.sh`; [ui-two-host.md](ui-two-host.md); `-mode ui`; `-mode discover`; `-mode discover-wait`; README UI and discovery commands; [discovery-matrix-20260519.md](discovery-matrix-20260519.md). | UI advertises Bonjour TXT metadata, samples Thunderbolt, AWDL, then LAN, targets the selected remote publisher when `-discover-peer` is set, and falls back when a higher-priority path returns no replies. `TestLinkHealthSamplePreferredFallsBackToAWDL` and `TestLinkHealthSamplePreferredSkipsUnavailableThunderbolt` cover the fallback rule. Live headless discovery fed LAN, Thunderbolt, and AWDL peer addresses into the two-host matrix. A local visible UI run with remote headless discovery showed a readable Thunderbolt active path, all three possible paths, bandwidth history, and `sampling thunderbolt` status after the text-layout fix. The 2026-05-20 published-module visible run captured active Thunderbolt and 0.0% loss history while targeting the current remote service. The matching direct probe passed Thunderbolt, AWDL, and LAN with 0.00% loss. | It measures UDP link health, not WebRTC datachannel health. Physical Thunderbolt removal is still required to prove visible Thunderbolt-to-AWDL fallback without restarting either process; unprivileged `ifconfig bridge0 down` was denied. |
...
| Use NotebookLM-assisted self-prompting | `/Users/tmc/go/src/github.com/tmc/skills/skills/notebooklm-assisted-self-prompting/SKILL.md`; `nlm source sync`; `nlm generate-chat`. | The repo state and new `/tmp` UI proof transcripts were synced to notebook `awdl-webrtc-apple-demo productization self-prompting 2026-05-19` as source `a2e2769b-c426-4805-9823-54c9f5e88ed9`. The drift check confirmed the docs were on mission and found no stale claims or required corrections before commit. | Continue using the anchor before any further broad design changes. |
| Keep repo hygiene and atomic history | Git commits and notes; `git status`; workspace gates. | Demo changes are split into atomic code commits with docs layered separately; `apple-pion` is clean; unrelated dirty files in `tmc/apple` are preserved instead of being reverted. | Push/publish policy is separate from this local workspace proof; current `go.work` gate is blocked until `../apple` is clean. |
...
| Reusable package split | `github.com/tmc/apple/x/network/nwpacket`; `github.com/tmc/apple-pion/nwtransport`; `github.com/tmc/apple-pion/icepolicy`; demo has checked-in `go.work`. | PASS for the package split and historical workspace proof | Current published-module demo gate passes; rerun the checked-in `go.work` gate after dirty `../apple` is clean. |
| Repo hygiene | Demo changes are split into atomic commits before docs; unrelated dirty files in `tmc/apple` are preserved. | PARTIAL | Current demo repo is clean before this docs update, but checked-in workspace verification is blocked by dirty `../apple`; see [workspace-gate-20260520.md](workspace-gate-20260520.md). |
...
two-host Bonjour control plane, live discovery-fed headless matrix, long
selected-link soak, local SwiftUI active-path rendering, selected-peer UI
targeting, the repeatable UI proof harness, and a published-module visible UI
active-path proof. The remaining product proof is physical Thunderbolt-removal
fallback observed in the SwiftUI window.
