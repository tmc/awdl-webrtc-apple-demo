...
| SwiftUI text layout and harness | PASS for rendering bug fix and launch harness: the local UI active-path run no longer uses zero-height text-cell frames, shortens long endpoints, and showed a readable Thunderbolt active path fed by a remote headless discovery publisher. `scripts/run-ui-harness.sh` now makes the remote publisher plus local UI launch repeatable for the physical proof, and preserves the checked-in workspace build by default while allowing `BUILD_GOWORK=off` for an explicit published-module UI proof. `LAUNCH_UI=0 ... scripts/run-ui-harness.sh` passed from clean commit `d7da303`, and `LAUNCH_UI=0 BUILD_GOWORK=off ... scripts/run-ui-harness.sh` passed from clean commit `1288332`, matching the current remote publisher and seeing LAN, Thunderbolt, and AWDL ready. This is layout and harness evidence only, not physical fallback completion. |
...
| Add SwiftUI link-health view with fallback | `ui.go`; `ui_test.go`; `scripts/run-ui-harness.sh`; [ui-two-host.md](ui-two-host.md); `-mode ui`; `-mode discover`; `-mode discover-wait`; README UI and discovery commands; [discovery-matrix-20260519.md](discovery-matrix-20260519.md). | UI advertises Bonjour TXT metadata, samples Thunderbolt, AWDL, then LAN, and falls back when a higher-priority path returns no replies. `TestLinkHealthSamplePreferredFallsBackToAWDL` and `TestLinkHealthSamplePreferredSkipsUnavailableThunderbolt` cover the fallback rule. Live headless discovery fed LAN, Thunderbolt, and AWDL peer addresses into the two-host matrix. A local visible UI run with remote headless discovery showed a readable Thunderbolt active path, all three possible paths, bandwidth history, and `sampling thunderbolt` status after the text-layout fix. The harness builds the current workspace binary by default, can build with `BUILD_GOWORK=off` for the current published-module fallback, installs it remotely, starts the headless publisher, runs local discovery preflight, opens the local UI by default, and both non-visual harness smokes passed against the remote Mac with all three paths ready. | It measures UDP link health, not WebRTC datachannel health. A two-live-Mac visual run with physical Thunderbolt removal is still required to prove visible Thunderbolt-to-AWDL fallback without restarting either process. |
...
selected-link soak, local SwiftUI active-path rendering, the repeatable UI
proof harness, and a non-visual harness smoke. The remaining product proof is
the two-live-Mac SwiftUI visual observation with physical Thunderbolt-removal
fallback.
