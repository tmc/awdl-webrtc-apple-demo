...
| Direction/asymmetry cleanup | `udp-callback-listen`, `udp-callback-request`, path policy checks, `nwpacket` readiness retries, `remote-diagnostics.sh`, matrix-local reachability diagnostics, per-listener UDP socket/route diagnostics, and a simultaneous bidirectional UDP matrix step are implemented. | PARTIAL | Run `SSH_TARGET=tmc2@10.0.18.249 scripts/remote-diagnostics.sh`, then `REQUIRE_PATHS=1 SSH_TARGET=tmc2@10.0.18.249 scripts/remote-matrix-bundle.sh`; inspect callback, local-to-remote UDP, bidirectional UDP, listener `lsof`/`netstat`, sender route sections, and the generated summary. |
| Performance hardening | `udp-perf` supports `-duration`, `-trials`, `-window`, `-streams`, `-packet-timeout`, `-listen-idle-timeout`, loss columns, JSON, latency mode, bidirectional matrix probing, Network.framework path records, and `cmd/matrix-summary` Markdown summaries with failure rows. | PARTIAL | Run longer two-host matrix samples with `DURATION`, `TRIALS`, `WINDOW`, and `STREAMS` after remote reachability returns using `scripts/remote-matrix-bundle.sh`, then inspect the generated transcript and summary. |
