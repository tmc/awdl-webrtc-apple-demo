...
| Keep backends pluggable | `-backend go\|network`; `-pion-net` switch from `SetICEUDPMux` to `SettingEngine.SetNet`; `github.com/tmc/apple-pion/nwtransport`; checked-in `go.work`. | Workspace `go test ./...` passes; LAN, Thunderbolt, and AWDL `-pion-net` WebRTC pass in the current matrix. | Broader all-Network.framework TCP/TURN/STUN ownership remains outside this demo. |
