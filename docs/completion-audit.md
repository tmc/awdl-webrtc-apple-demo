...
| Performance hardening | `udp-perf` supports `-duration`, `-trials`, `-window`, `-streams`, `-packet-timeout`, `-listen-idle-timeout`, loss columns, JSON, latency mode, and Network.framework path records. | PARTIAL | Run longer two-host matrix samples with `DURATION`, `TRIALS`, `WINDOW`, and `STREAMS` after remote reachability returns. |
