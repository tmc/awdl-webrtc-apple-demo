...
| Current peer reachability | BLOCKED on 2026-05-19: the latest fresh check reached `10.0.18.249` by ping, but TCP/22 returned `Connection refused` and SSH exited 255. Earlier Bonjour and soak attempts also stopped at peer reachability; their route/ping/TCP/22 diagnostics are preserved in `/tmp/awdl-webrtc-bonjour-unreachable-20260519.txt` and `/tmp/awdl-webrtc-dry-run-test.txt`. |
...
| Cover Thunderbolt, AWDL, and LAN tests | `scripts/remote-matrix.sh`; `scripts/remote-matrix-bundle.sh`; path requirements; optional `USE_DISCOVERY=1` or `DISCOVERY_FILE` peer addresses. | Current matrix covers all three profiles and passes WebRTC, UDP perf, latency, and callback probes with path evidence. Discovery-file parsing was exercised locally and populated LAN, Thunderbolt, and AWDL peer addrs before the current SSH reachability blocker. | The discovery-fed matrix mode still needs a two-live-Mac run. |
