...
| SwiftUI text layout | PASS for rendering bug fix: the local UI active-path run no longer uses zero-height text-cell frames, shortens long endpoints, and showed a readable Thunderbolt active path fed by a remote headless discovery publisher. `scripts/run-ui-harness.sh` now makes the remote publisher plus local UI launch repeatable for the physical proof. This is layout and harness evidence only, not physical fallback completion. |
...
selected-link soak, local SwiftUI active-path rendering, and the repeatable UI
proof harness. The remaining product proof is the two-live-Mac SwiftUI visual
observation with physical Thunderbolt-removal fallback.
