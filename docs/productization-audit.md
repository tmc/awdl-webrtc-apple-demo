...
| Durable AWDL/link-local candidate handling | PARTIAL | `github.com/tmc/apple-pion/icepolicy` now owns the AWDL raw-host-candidate policy, with package tests and examples. The demo keeps SDP unmodified and publishes explicit `ICECandidateInit` records with the selected link-local IP. AWDL raw-candidate gather passes. A direct `SetNAT1To1IPs(fe80/fe80)` experiment gathered zero AWDL candidates. | Re-run two-host raw-candidate WebRTC with `tmc2@10.0.18.249` once SSH is reachable again. |
...
| Reusable package | PARTIAL | `tmc/apple` release `v0.6.7` provides `github.com/tmc/apple/x/network/nwpacket` with `PathReporter` and outbound readiness retry knobs; local `tmc/apple-pion` adds `github.com/tmc/apple-pion/nwtransport` and `github.com/tmc/apple-pion/icepolicy`; the demo imports `apple-pion` through one remaining local replace. | Publish or relocate `apple-pion`, remove the demo's `replace github.com/tmc/apple-pion => ../apple-pion`, and verify the demo resolves `apple-pion` from the module cache. |
| Published module graph | PARTIAL | `scripts/release-preflight.sh` verifies local tests, vet, script syntax, published `tmc/apple`, published `apple-pion`, clean owned worktrees, configured remotes, published HEADs, and absence of local replaces. It currently passes the local gates and fails on the intended blockers. | `github.com/tmc/apple-pion` and `github.com/tmc/awdl-webrtc-apple-demo` do not currently exist as GitHub repos, the local checkouts have no remotes, and `tmc2@10.0.18.249` is unreachable for fresh remote validation. |
...
with explicit raw candidate publication in earlier two-host runs. The remaining
productization boundary is `apple-pion` release or relocation, demo release
setup, fresh remote reruns, longer performance samples, and UDP asymmetry
cleanup once `tmc2@10.0.18.249` is reachable.
