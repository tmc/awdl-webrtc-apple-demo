...
| --- | --- | --- | --- |
| Full Pion-native backend closer to `transport.Net` | PARTIAL | `nwtransport` implements Pion `transport.Net` for UDP host listeners and concrete UDP dials through `nwpacket`; `-pion-net -mdns disabled -raw-candidates` remote WebRTC passes on LAN, Thunderbolt, and AWDL. | Decide whether TCP, wildcard UDP, TURN/STUN, and package placement stay fallback-backed or become Network.framework-native. |
...
| Reusable package | PASS | `tmc/apple` commit `ec3a7fea` adds `github.com/tmc/apple/network/nwpacket`; this demo imports it through `replace github.com/tmc/apple => ../apple`; `nwtransport` remains demo-local. | Cut an Apple module release and remove the local replace. |
