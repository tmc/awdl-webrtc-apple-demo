...
| Reusable package | PASS | `tmc/apple` commit `eb350571` adds `github.com/tmc/apple/x/network/nwpacket`, now consumed as released `github.com/tmc/apple v0.6.4`; `tmc/apple-pion` adds `github.com/tmc/apple-pion/nwtransport` and `github.com/tmc/apple-pion/icepolicy`; this demo imports `apple-pion` through one remaining local replace. | Give `apple-pion` a remote/release and remove the remaining local replace. |
