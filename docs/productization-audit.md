...
| Reusable package | PASS | `tmc/apple` release `v0.6.6` provides `github.com/tmc/apple/x/network/nwpacket` with `PathReporter`; `tmc/apple-pion` adds `github.com/tmc/apple-pion/nwtransport` and `github.com/tmc/apple-pion/icepolicy`; this demo imports `apple-pion` through one remaining local replace. | Give `apple-pion` a remote/release and remove the remaining local replace. |
