...
| Performance hardening | PARTIAL | `udp-perf` and `udp-perf-send` support `-trials`, aggregate trial summaries, and bounded in-flight `-window`; output includes `Lost` and `Loss`; `-perf-json` emits per-trial and aggregate machine-readable result records including `window`; `-packet-timeout` counts echo timeouts as lost datagrams; local and two-host Network.framework smoke tests pass. | Run longer repeated remote trials and tune the window/backpressure behavior with the second host once reachable. |
