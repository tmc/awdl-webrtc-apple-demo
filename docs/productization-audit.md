...
| Published module graph | PARTIAL | `scripts/release-preflight.sh` verifies local tests, vet, script syntax, published `tmc/apple`, published `apple-pion`, clean owned worktrees, configured remotes, published HEADs, and absence of local replaces. The demo now depends on `github.com/tmc/apple-pion v0.1.0` without a replace. | Publish the demo repository HEAD, then re-run preflight with GitHub/DNS reachability. `tmc2@10.0.18.249` is still unreachable for fresh remote validation. |
...
productization boundary is demo release setup, fresh remote reruns, longer
performance samples, and UDP asymmetry cleanup once `tmc2@10.0.18.249` is
reachable.
