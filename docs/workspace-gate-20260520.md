...
| UI harness syntax | PASS | `test -x scripts/run-ui-harness.sh && bash -n scripts/run-ui-harness.sh` passed. |
...
for current demo verification, and rerun the checked-in `go.work` gate only
