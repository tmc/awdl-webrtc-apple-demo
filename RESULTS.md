...
| Pion changes | PASS | No Pion fork or patch was needed. | Uses `SettingEngine` filters, mDNS mode, `SetICEUDPMux`, `SetNet`, explicit signaling, and `internal/icepolicy`. | AWDL raw-candidate publication is now explicit candidate signaling; fresh two-host rerun is pending while SSH is unreachable. |
| Pluggable backend shape | PASS | The CLI has `-backend go\|network`; WebRTC can use either an ICE UDP mux or `-pion-net`. | `go test ./...`; `-backend network` gather, UDP perf, remote WebRTC, and `-pion-net` LAN/Thunderbolt/AWDL remote WebRTC pass. | TCP, TURN/STUN, release placement, and candidate publication are still product decisions. |
| Reusable packet package | PASS | The Network.framework PacketConn lives at `github.com/tmc/apple/x/network/nwpacket`, and the demo imports it through `apple-pion`. | `tmc/apple` commit `eb350571` adds package docs, examples, and tests; `go test ./x/network/nwpacket` and `go vet ./x/network/nwpacket` pass there. | The demo uses `replace github.com/tmc/apple => ../apple` until an Apple module release carries the package. |
| Pion `transport.Net` package | PASS | `github.com/tmc/apple-pion/nwtransport` implements the Pion `transport.Net` UDP surface needed by ICE, and the demo imports it. | `tmc/apple-pion` commits `d2698d5`, `167087c`, and `3ea6faf` add package docs, tests, an example, UDP dials, concrete UDP listens, configured wildcard UDP listens, and the current `x/network/nwpacket` import path; LAN, Thunderbolt, and AWDL `-pion-net -mdns disabled -raw-candidates offer-ssh` opened datachannels and exchanged `ping`/`pong`. | TCP, DNS, unconstrained wildcard UDP, and unsupported UDP cases intentionally fall back to Pion's standard network. |
| Explicit ICE policy | PARTIAL | AWDL host-candidate handling is factored into `internal/icepolicy`, and raw candidates are signaled separately from SDP. | Commit `d1ef7ad` strips candidates from raw-link SDP, publishes `ICECandidateInit` records with the selected link-local IP, keeps legacy wire decode support, and tests the wire shape; AWDL raw-candidate gather passes. | Fresh two-host AWDL/Thunderbolt/LAN rerun is pending while `tmc2@10.0.18.249` is unreachable. |
...
| UDP perf output | PASS | The demo prints iperf-like UDP summaries with loss columns, repeated trials, optional JSON records, sender-side timeout loss accounting, and a bounded in-flight window. | Columns: interval, transfer, bitrate, datagrams, lost, loss, omit, RTT min/avg/p50/p95/max; `-trials` repeats runs; `-window` enables pipelined echo requests; `-perf-json` emits JSON result records; `-packet-timeout` bounds each echo wait. | Smoke benchmark, not an `iperf3` replacement. |
...
| Network repeated perf smoke | `go run . -profile lan -backend network -mode udp-perf -count 2 -warmup 0 -trials 2 -perf-json -timeout 15s` | Trial summaries print with `Lost` and `Loss` columns; `-perf-json` emits machine-readable result records. A no-listener sender smoke counted 2/2 lost datagrams instead of aborting. |
| Network pipelined perf smoke | `go run . -profile lan -backend network -mode udp-perf -count 4 -warmup 0 -window 2 -perf-json -timeout 10s` | Same-host Network.framework UDP completed with 4 datagrams, zero loss, and JSON including `"window":2`. |
...
| Two-host AWDL perf sender | `go run . -profile awdl -mode udp-perf-send -peer '[fe80::peer%awdl0]:12345' -count 1000 -size 1200 -warmup 5 -trials 3 -window 4 -perf-json -timeout 20s` | Prints iperf-like transfer, bitrate, datagrams, loss, omit, and RTT summary for each trial plus JSON records including the in-flight window. |
