...
| Bonjour signaling smoke | `GOWORK=off go run . -mode answer-bonjour -signal-name codex-smoke12 -profile lan -backend network -pion-net -mdns disabled -candidate-policy raw -timeout 25s` plus `offer-bonjour` exchanged `OFFER`/`ANSWER` via Bonjour signaling. The datachannel then timed out in ICE checking with local and remote `10.0.199.147` host candidates and candidate-pair stats showing requests but zero responses. | This validates the Bonjour control-plane shape locally but not same-host WebRTC datachannel success. |
...
| Bonjour WebRTC signaling | Answer side: `go run . -profile awdl -backend network -pion-net -mdns disabled -candidate-policy auto -mode answer-bonjour -signal-name awdl-webrtc-peer-a -timeout 90s`; offer side: same flags with `-mode offer-bonjour -signal-peer awdl-webrtc-peer-a` | IMPLEMENTED BUT UNPROVEN ON TWO HOSTS: advertises `_awdl-webrtc-signal._tcp` and uses Network.framework browse plus `NSNetService` host/port fallback for the same `OFFER`/`ANSWER` wire format. Local same-host signaling exchanged the lines; same-host ICE still timed out. |
