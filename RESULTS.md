...
| Area | Current result |
| --- | --- |
| Published backend pins | `github.com/tmc/apple v0.6.7`; `github.com/tmc/apple-pion v0.1.3`; no local replace. The most recent two-host matrix below was run against `apple-pion v0.1.1`. |
...
| Non-SSH WebRTC signaling | PARTIAL | Manual stdio signaling is implemented, and experimental Network.framework Bonjour signaling now advertises/browses/dials a peer service for the same wire format. | `offer-stdio`/`answer-stdio`; `offer-bonjour`/`answer-bonjour`; parser tests pass; Bonjour smoke advertised `_awdl-webrtc-signal._tcp`, `dns-sd -B` and `dns-sd -L` saw the service, and `nc` reached the advertised host:port. | Same-host `offer-bonjour` still times out with `last_state=NWConnectionStatePreparing`; two-host Bonjour signaling remains unproven while the peer is unreachable. |
...
| Remote SSH | UNSTABLE: the 2026-05-19 retry reached `10.0.18.249` by ICMP over `en0`, waited through initial TCP/22 timeouts, copied the binary after SSH recovered briefly, then saw later SSH commands fail with `Operation timed out`, `Host is down`, and `No route to host`. |
| Remote matrix retry | `WEBRTC_TRACE=1 REMOTE_READY_TIMEOUT=30 CANDIDATE_POLICY=auto REQUIRE_PATHS=1 SSH_TARGET=tmc2@10.0.18.249 REMOTE_BIN=/Volumes/Shared/awdl-webrtc-apple-demo-bin OUTPUT=/tmp/awdl-webrtc-matrix-try-again.txt SUMMARY_OUTPUT=/tmp/awdl-webrtc-matrix-try-again.md scripts/remote-matrix-bundle.sh` wrote [docs/matrix-try-again-20260519.md](docs/matrix-try-again-20260519.md). It reached LAN Pion candidate exchange, but that WebRTC leg failed ICE; all later UDP/WebRTC probes failed because the remote SSH control plane dropped before remote sender/listener processes could start. |
...
| Bonjour signaling smoke | `GOWORK=off go run . -mode answer-bonjour -signal-name codex-smoke11 -profile lan -backend network -pion-net -mdns disabled -candidate-policy raw -timeout 12s` plus `offer-bonjour` found the service but timed out waiting for Network.framework TCP readiness with `last_state=NWConnectionStatePreparing`. A simultaneous `dns-sd -B _awdl-webrtc-signal._tcp local.` and `dns-sd -L codex-dnssd1 _awdl-webrtc-signal._tcp local.` resolved the service, and `nc -vz` to the advertised host:port succeeded. This isolates the local blocker to Network.framework self-dialing the Bonjour endpoint, not advertisement. |
...
| Remote | `tmc2@10.0.18.249`; TCP/22 currently blocked | `10.0.18.249` | `169.254.88.35` | `fe80::9477:6dff:fe11:6a55%awdl0` |
...
| Remote productization retry | `WEBRTC_TRACE=1 REMOTE_READY_TIMEOUT=30 CANDIDATE_POLICY=auto REQUIRE_PATHS=1 SSH_TARGET=tmc2@10.0.18.249 REMOTE_BIN=/Volumes/Shared/awdl-webrtc-apple-demo-bin OUTPUT=/tmp/awdl-webrtc-matrix-try-again.txt SUMMARY_OUTPUT=/tmp/awdl-webrtc-matrix-try-again.md scripts/remote-matrix-bundle.sh` | Current result: [docs/matrix-try-again-20260519.md](docs/matrix-try-again-20260519.md). SSH recovered briefly enough to copy the binary and start LAN WebRTC, then dropped during the matrix. LAN Pion exchanged candidates but failed ICE; LAN/Thunderbolt/AWDL UDP rows mostly report zero datagrams because remote commands did not start. |
...
| Bonjour WebRTC signaling | Answer side: `go run . -profile awdl -backend network -pion-net -mdns disabled -candidate-policy auto -mode answer-bonjour -signal-name awdl-webrtc-peer-a -timeout 90s`; offer side: same flags with `-mode offer-bonjour -signal-peer awdl-webrtc-peer-a` | IMPLEMENTED BUT UNPROVEN: advertises `_awdl-webrtc-signal._tcp` and uses Network.framework browse/TCP for the same `OFFER`/`ANSWER` wire format. Local same-host discovery resolves, but the Network.framework Bonjour connection remains in `Preparing`; needs a two-Mac run. |
