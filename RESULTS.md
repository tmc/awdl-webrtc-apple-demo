...
| Productized packages | `go test ./nwtransport ./internal/icepolicy`; in `tmc/apple`: `go test ./network/nwpacket` | `nwtransport`, `internal/icepolicy`, and promoted `nwpacket` tests pass. |
| Network backend build pin | `go list -m -f '{{.Path}} {{.Version}} {{.Replace.Path}}' github.com/tmc/apple` | `github.com/tmc/apple v0.6.3 ../apple`; local replace points at `tmc/apple` commit `ec3a7fea`. |
