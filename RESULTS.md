...
| Area | Current result |
| --- | --- |
| Dependency mode | The 2026-05-19 matrix used the checked-in `go.work` with `../apple` and `../apple-pion`; no new tags were published for that run. Current 2026-05-20 local verification uses `GOWORK=off` because dirty `../apple` blocks the workspace gate. See [docs/workspace-gate-20260520.md](docs/workspace-gate-20260520.md). |
...
| Bonjour control plane | PASS: [docs/bonjour-live-20260519.md](docs/bonjour-live-20260519.md) records LAN, Thunderbolt, and AWDL `signal` and `full` Bonjour WebRTC phases opening datachannels. |
| Discovery-fed matrix | PASS: [docs/discovery-matrix-20260519.md](docs/discovery-matrix-20260519.md) records live Bonjour/TXT peer addresses feeding LAN, Thunderbolt, and AWDL matrix probes. |
| Long soak | PASS for selected-link sweeps: [docs/soak-live-20260519.md](docs/soak-live-20260519.md) records 30s x 5-trial x 2-stream LAN, Thunderbolt, and AWDL throughput/latency tables. The same run exposed and fixed the old long discovery-publisher timeout. |
| SwiftUI link monitor | PARTIAL: [docs/ui-two-host.md](docs/ui-two-host.md) records readable local UI active-path runs fed by a remote headless discovery publisher. The latest `BUILD_GOWORK=off` visible run captured `/tmp/awdl-webrtc-ui-rerun-focused-20260520.png` with Thunderbolt selected, peer `MacBook-m4small.local`, LAN/Thunderbolt/AWDL possible paths, and 0.0% loss bandwidth history. [scripts/run-ui-harness.sh](scripts/run-ui-harness.sh) now starts the remote publisher, runs local discovery preflight, passes the selected peer into the UI, and opens the local UI for the physical proof. It builds through the checked-in workspace by default and accepts `BUILD_GOWORK=off` for the current published-module fallback. Its non-visual smokes passed from clean commits `d7da303` and `1288332`, and `/tmp/awdl-webrtc-ui-direct-probe-20260520.txt` proves the same published-module build can sample Thunderbolt, AWDL, and LAN to the remote publisher with 0.00% loss. Physical Thunderbolt-removal fallback is still not visually proven because unprivileged `ifconfig bridge0 down` was denied. |
...
| Build | PASS | The module builds and the focused tests pass against published module versions. | `GOWORK=off go test ./...`; `GOWORK=off go vet ./...`; `git diff --check`. | Darwin-only demo. Current checked-in `go.work` gate is blocked by dirty sibling `../apple`. |
| WebRTC support | PASS | Yes, via Pion WebRTC with constrained ICE. | Modes: `check`, `gather`, `pair`, `answer-stdio`, `offer-stdio`, `answer-bonjour`, `offer-bonjour`, `offer-ssh`; [docs/bonjour-live-20260519.md](docs/bonjour-live-20260519.md) records Bonjour `signal` and `full` datachannel proof for LAN, Thunderbolt, and AWDL. | Remaining WebRTC backend work is the broader all-Network.framework DNS/TCP/TURN/STUN surface, not this selected UDP proof. |
...
| Pluggable backend shape | PASS | The CLI has `-backend go\|network`; WebRTC can use either an ICE UDP mux or `-pion-net`. | Current `GOWORK=off go test ./...` passes; `-backend network` gather, UDP perf, remote WebRTC, and `-pion-net` LAN/Thunderbolt/AWDL remote WebRTC pass in the workspace-built matrix. | A broader all-Network.framework DNS/TCP/TURN/STUN backend remains outside this demo slice. |
| Reusable packet package | PASS for the proof | The Network.framework PacketConn lives at `github.com/tmc/apple/x/network/nwpacket`, and this demo consumed the sibling checkout through `go.work` for the 2026-05-19 workspace matrix. | The workspace matrix proves the local PacketConn changes across LAN, Thunderbolt, and AWDL; current `GOWORK=off` demo tests pass against published `github.com/tmc/apple v0.6.7`. | Current checked-in workspace verification is blocked by dirty `../apple`; keep release-tag checks separate from workspace proof runs. |
| Pion `transport.Net` package | PASS | `github.com/tmc/apple-pion/nwtransport` implements the Pion `transport.Net` UDP surface needed by ICE, and the demo consumed the sibling checkout through `go.work` for the workspace matrix. | Current LAN, Thunderbolt, and AWDL `-pion-net -mdns disabled -candidate-policy auto offer-ssh` opened datachannels and exchanged `ping`/`pong`; `../apple-pion` is clean and its tests assert native numeric UDP handling, named UDP fallback, and `CreateListenConfig.ListenPacket` native wildcard behavior. | DNS, TCP, unconstrained wildcard UDP, named UDP endpoints, TURN/STUN helper traffic outside the selected UDP surface, and unsupported UDP cases intentionally fall back to Pion's standard network. |
...
| Non-SSH WebRTC signaling | PASS | Manual stdio signaling is implemented, and Network.framework Bonjour signaling now advertises/browses a peer service for the same wire format. | `offer-stdio`/`answer-stdio`; `offer-bonjour`/`answer-bonjour`; parser tests pass; [docs/bonjour-live-20260519.md](docs/bonjour-live-20260519.md) records two-host Bonjour `signal` and `full` phases passing for LAN, Thunderbolt, and AWDL. SSH was only the process-control channel. | Same-host full Bonjour still times out after ICE checking, so use two Macs for real link proof. |
| Headless discovery | PASS | The Bonjour/TXT discovery path used by the SwiftUI monitor is also available as JSON output and can feed the remote harnesses. | Local discovery emitted `link_health_discovery` records, and [docs/discovery-matrix-20260519.md](docs/discovery-matrix-20260519.md) records live discovery-fed LAN, Thunderbolt, and AWDL peer addresses consumed by `scripts/remote-matrix.sh` through `USE_DISCOVERY=1`. | The remaining UI gap is visual observation and physical cable-removal fallback, not headless peer discovery. |
| UI fallback selection | PARTIAL | The link-health sampler tries Thunderbolt, then AWDL, then LAN. The rendered monitor now keeps rows readable instead of collapsing text, and `scripts/run-ui-harness.sh` provides a repeatable two-Mac launch path for the manual proof. | `TestLinkHealthSamplePreferredFallsBackToAWDL` and `TestLinkHealthSamplePreferredSkipsUnavailableThunderbolt` pass; two-host headless discovery found live LAN, Thunderbolt, and AWDL paths; the 2026-05-20 visible UI run showed the selected remote peer, active Thunderbolt, all three possible paths, and 0.0% loss history; the matching direct probe passed Thunderbolt/AWDL/LAN UDP with path evidence. | Physical Thunderbolt removal still needs a human cable pull or privileged link toggle; unprivileged `ifconfig bridge0 down` returned `permission denied`. |
| Long soak wrapper | PASS | Longer repeated sweeps have a dedicated wrapper and live selected-link evidence. | `scripts/remote-soak.sh` sets 30s duration, 5 trials, 2 streams, path requirements, WebRTC retries, and timestamped transcript/summary outputs; [docs/soak-live-20260519.md](docs/soak-live-20260519.md) records LAN, Thunderbolt, and AWDL selected-link soak tables. | The old long discovery-fed probe failed because the publisher expired at 60s; `scripts/remote-matrix.sh` now defaults `DISCOVERY_PUBLISH_TIMEOUT=2h`, and a 70s LAN discovery-fed smoke passed. |
...
| Remote SSH | PASS for the current proof set: SSH process control stayed available during the workspace matrix, Bonjour proof, discovery-fed matrix, soak run, and discovery-lifetime smoke. Earlier SSH-control-plane failures are preserved in [docs/matrix-try-again-20260519.md](docs/matrix-try-again-20260519.md) as historical setup diagnostics. |
...
| Local SwiftUI active-path view | `AWDL_DEMO_NETWORK_TRACE=1 BUILD_GOWORK=off SSH_TARGET=tmc2@10.0.18.249 OUTPUT=/tmp/awdl-webrtc-ui-harness-physical-rerun-20260520.txt REMOTE_LOG=/tmp/awdl-webrtc-ui-harness-physical-rerun-remote-20260520.log scripts/run-ui-harness.sh` | The rendered UI in `/tmp/awdl-webrtc-ui-rerun-focused-20260520.png` showed active path `thunderbolt`, peer `MacBook-m4small.local`, possible paths for Thunderbolt/AWDL/LAN, and bandwidth history rows with 0.0% loss. This verifies selected-peer UI targeting, the text-layout fix, and active-path visualization from the published module graph. |
...
| Bonjour signaling smoke | `go run . -mode answer-bonjour -signal-name codex-signalonly1 -signal-only -profile lan -backend network -pion-net -mdns disabled -candidate-policy raw -timeout 20s` plus matching `offer-bonjour -signal-only` exchanged `OFFER`/`ANSWER` via Bonjour signaling and exited 0 on both sides. This validates the Bonjour control-plane shape locally; the two-host harness below is the datachannel proof. |
| Two-host Bonjour harness | `REMOTE_READY_TIMEOUT=30 REMOTE_STEP_READY_TIMEOUT=30 PROFILES="lan thunderbolt awdl" PHASES="signal full" SSH_TARGET=tmc2@10.0.18.249 OUTPUT=/tmp/awdl-webrtc-bonjour-live2-20260519.txt scripts/remote-bonjour.sh` passed. [docs/bonjour-live-20260519.md](docs/bonjour-live-20260519.md) records LAN, Thunderbolt, and AWDL `signal` and `full` phases passing, including datachannel `ping`/`pong` over `en0`, `bridge0`, and `awdl0`. |
...
| Discovery-fed matrix | `USE_DISCOVERY=1 CANDIDATE_POLICY=auto REQUIRE_PATHS=1 ... OUTPUT=/tmp/awdl-webrtc-discovery-matrix-20260519.txt SUMMARY_OUTPUT=/tmp/awdl-webrtc-discovery-matrix-20260519.md scripts/remote-matrix-bundle.sh` passed. [docs/discovery-matrix-20260519.md](docs/discovery-matrix-20260519.md) records discovery-sourced LAN `10.0.18.249`, Thunderbolt `169.254.82.92`, and AWDL `[fe80::4c41:acff:fec5:96f1%awdl0]` peer addrs plus WebRTC pass on all three profiles. |
| Matrix reachability diagnostics | The current matrix captured route-to-peer on `en0`, ping success, TCP/22 success, `scutil --nwi`, local interface inventory, and then continued into all LAN/Thunderbolt/AWDL probes. Discovery parsing was locally exercised with `/tmp/awdl-discovery-fake.json`; live discovery-fed parsing and probes passed in [docs/discovery-matrix-20260519.md](docs/discovery-matrix-20260519.md). |
...
| --- | --- | --- |
| Build gate | `GOWORK=off go test ./...`; `GOWORK=off go vet ./...`; `git diff --check` | Current published-module demo tests pass with `github.com/tmc/apple v0.6.7` and `github.com/tmc/apple-pion v0.1.3`; vet and whitespace checks pass. |
| Productized packages | In `tmc/apple`: historical `go test ./x/network/nwpacket`; in `tmc/apple-pion`: `go test ./...` | Promoted `nwpacket`, `nwtransport`, and `icepolicy` tests passed for the workspace proof; `../apple-pion` remains clean. Current `../apple` is dirty and not repaired by this demo docs pass. |
| Workspace backend source | `cat go.work`; `go env GOWORK`; `go test ./...` | `go.work` uses `.`, `../apple`, and `../apple-pion`; no new tags were published for the current proof. Current `go test ./...` is blocked by dirty `../apple/foundation/nsuuid.gen.go` referencing undefined `kernel.Uuid_t`; see [docs/workspace-gate-20260520.md](docs/workspace-gate-20260520.md). |
...
| Manual WebRTC signaling | Offer side: `go run . -profile awdl -backend network -pion-net -mdns disabled -candidate-policy auto -mode offer-stdio -timeout 90s`; answer side: same flags with `-mode answer-stdio` | IMPLEMENTED: uses the same `OFFER`/`ANSWER` wire format and explicit candidate policy as `offer-ssh`, but does not require SSH. Bonjour is the completed automated non-SSH two-host proof. |
| Bonjour WebRTC signaling | Answer side: `go run . -profile awdl -backend network -pion-net -mdns disabled -candidate-policy auto -mode answer-bonjour -signal-name awdl-webrtc-peer-a -timeout 90s`; offer side: same flags with `-mode offer-bonjour -signal-peer awdl-webrtc-peer-a`; add `-signal-only` on both sides for control-plane-only proof. | PROVEN ON TWO HOSTS: [docs/bonjour-live-20260519.md](docs/bonjour-live-20260519.md) records LAN, Thunderbolt, and AWDL `signal` and `full` phases passing over `_awdl-webrtc-signal._tcp`. |
| Headless link discovery | `go run . -mode discover -backend network -timeout 10s -ui-interval 1s` | Emits JSON records with local Thunderbolt, AWDL, and LAN listener addresses plus newest peer metadata from Network.framework Bonjour browse results, including advertised version, commit, and supported modes when available. Live discovery-fed matrix proof is in [docs/discovery-matrix-20260519.md](docs/discovery-matrix-20260519.md). |
| Wait for discovered peer | `go run . -mode discover-wait -backend network -timeout 30s`; optional `-discover-peer peer-id-or-name` | Emits one JSON record and exits when a matching peer is discovered; setup diagnostics go to stderr so stdout remains JSON. `USE_DISCOVERY=1` matrix runs consume that peer record directly. |
...
Long selected-link soak samples:
...
The full long-soak table, including bidirectional and latency-only rows, is in
[docs/soak-live-20260519.md](docs/soak-live-20260519.md).
