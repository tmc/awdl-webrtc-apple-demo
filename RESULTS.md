...
| Pion `transport.Net` package | PASS | `github.com/tmc/apple-pion/nwtransport` implements the Pion `transport.Net` UDP surface needed by ICE, and the demo imports it. | `tmc/apple-pion` commits `d2698d5` and `167087c` add package docs, tests, an example, UDP dials, concrete UDP listens, and configured wildcard UDP listens; LAN, Thunderbolt, and AWDL `-pion-net -mdns disabled -raw-candidates offer-ssh` opened datachannels and exchanged `ping`/`pong`. | TCP, DNS, unconstrained wildcard UDP, and unsupported UDP cases intentionally fall back to Pion's standard network. |
...
| UDP perf output | PASS | The demo prints iperf-like UDP summaries with loss columns, repeated trials, optional JSON records, and real sender-side timeout loss accounting. | Columns: interval, transfer, bitrate, datagrams, lost, loss, omit, RTT min/avg/p50/p95/max; `-trials` repeats runs; `-perf-json` emits JSON result records; `-packet-timeout` bounds each echo wait. | Smoke benchmark, not an `iperf3` replacement. |
...
| Two-host AWDL perf sender | `go run . -profile awdl -mode udp-perf-send -peer '[fe80::peer%awdl0]:12345' -count 1000 -size 1200 -warmup 5 -trials 3 -perf-json -timeout 20s` | Prints iperf-like transfer, bitrate, datagrams, loss, omit, and RTT summary for each trial plus JSON records. |
