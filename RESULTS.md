...
| UDP perf output | PASS | The demo prints iperf-like UDP summaries with loss columns, repeated trials, and optional JSON records. | Columns: interval, transfer, bitrate, datagrams, lost, loss, omit, RTT min/avg/p50/p95/max; `-trials` repeats runs; `-perf-json` emits JSON result records. | Smoke benchmark, not an `iperf3` replacement. |
...
| Network repeated perf smoke | `go run . -profile lan -backend network -mode udp-perf -count 2 -warmup 0 -trials 2 -perf-json -timeout 15s` | Trial summaries print with `Lost` and `Loss` columns; `-perf-json` emits machine-readable result records. |
