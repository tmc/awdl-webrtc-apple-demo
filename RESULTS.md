...
| Pion changes | PASS | No Pion fork or patch was needed. | Uses `SettingEngine` filters, mDNS mode, `SetICEUDPMux`, `SetNet`, explicit signaling, and `apple-pion/icepolicy`. | AWDL link-local candidate publication is now explicit candidate signaling through `-candidate-policy auto` or `raw`; fresh two-host rerun is pending while SSH is unreachable. |
...
| Pion `transport.Net` package | PASS | `github.com/tmc/apple-pion/nwtransport` implements the Pion `transport.Net` UDP surface needed by ICE, and the demo imports it. | `tmc/apple-pion` commits `d2698d5`, `167087c`, `3ea6faf`, and `7d4aced` add package docs, tests, an example, UDP dials, concrete UDP listens, configured wildcard UDP listens, the current `x/network/nwpacket` import path, and test-covered fallback policy; LAN, Thunderbolt, and AWDL historical `-pion-net -mdns disabled -raw-candidates offer-ssh` opened datachannels and exchanged `ping`/`pong`; current auto-policy local gathers pass. | DNS, TCP, unconstrained wildcard UDP, TURN/STUN helper traffic outside the selected UDP surface, and unsupported UDP cases intentionally fall back to Pion's standard network. |
| Explicit ICE policy | PARTIAL | AWDL host-candidate handling is factored into `github.com/tmc/apple-pion/icepolicy`, raw candidates are signaled separately from unmodified SDP, and the demo now exposes `-candidate-policy auto\|mdns\|raw`. | `apple-pion/icepolicy` has package docs, tests, and examples for synthetic host-candidate publication and `ICECandidateInit` extraction; `TestNewCandidatePolicyConfig`; `go run . -profile awdl -backend network -pion-net -mdns disabled -mode gather -timeout 15s` gathered two raw `fe80::...` candidates with `candidate_policy=auto raw_candidates=true`. | Fresh two-host AWDL/Thunderbolt/LAN rerun is pending while `tmc2@10.0.18.249` is unreachable. |
...
| Network.framework remote WebRTC | PASS | Remote Pion datachannel exchange works over Network.framework PacketConn. | LAN, Thunderbolt, and AWDL `offer-ssh` each opened a datachannel and exchanged `ping`/`pong`. | AWDL requires `-mdns disabled` plus explicit link-local candidate publication; fresh rerun should use `-candidate-policy auto`. |
| Network.framework Pion-native remote WebRTC | PASS | Remote WebRTC through `SettingEngine.SetNet` works for LAN, Thunderbolt, and AWDL. | LAN, Thunderbolt, and AWDL historical `-pion-net -mdns disabled -raw-candidates offer-ssh` opened datachannels and exchanged `ping`/`pong`. | AWDL still depends on explicit link-local candidate publication; fresh rerun should use `-candidate-policy auto`. |
...
| AWDL WebRTC datachannel | PASS | Remote datachannel exchange works over Network.framework on `awdl0`. | Historical `go run . -profile awdl -backend network -mdns disabled -raw-candidates -mode offer-ssh ...` opened and exchanged payload with `tmc2@10.0.18.249`; current code preserves raw gather with explicit `-candidate-policy auto` signaling. | Fresh remote rerun of the auto-policy path is pending. |
...
| Matrix reachability diagnostics | `CONNECT_TIMEOUT=1 PROFILES=lan CANDIDATE_POLICY=auto OUTPUT=/tmp/awdl-webrtc-matrix-bundle-smoke.txt SUMMARY_OUTPUT=/tmp/awdl-webrtc-matrix-bundle-smoke.md scripts/remote-matrix-bundle.sh` | Before the SSH timeout, the matrix transcript captured `candidate_policy=auto`, route-to-peer via gateway `172.20.10.1` on `en0`, 100% ping loss, TCP/22 timeout, `scutil --nwi`, local interface inventory, and `FAIL: remote reachability exit=255`; the bundle still wrote a summary with a `remote reachability` failure row and returned matrix exit 255. |
| Matrix summary table | `go run ./cmd/matrix-summary /tmp/awdl-webrtc-matrix.txt` | Converts matrix `udp_perf`, `udp_perf_summary`, `udp_perf_listen`, `udp_latency`, and `udp_latency_summary` JSON lines plus de-duplicated `FAIL:` lines into a compact Markdown table with section, datagrams, loss, bitrate, RTT, path, and failure rows. |
...
| Release preflight | `scripts/release-preflight.sh` | Passes: local gates pass; `github.com/tmc/apple v0.6.7` and `github.com/tmc/apple-pion v0.1.0` resolve from the module cache; demo, `apple-pion`, and `apple` HEADs are published at `origin/main`; no local replace remains. | This host's default DNS is currently flaky; use `GITHUB_RESOLVE_IP=140.82.116.3 scripts/release-preflight.sh` when needed. |
| Remote productization matrix | `SSH_TARGET=tmc2@10.0.18.249 OUTPUT=/tmp/awdl-webrtc-matrix.txt SUMMARY_OUTPUT=/tmp/awdl-webrtc-matrix-summary.md scripts/remote-matrix-bundle.sh` | Prints the matrix config, records local route/ping/TCP reachability diagnostics for the SSH host, checks SSH reachability, builds and copies the demo, then runs LAN/Thunderbolt/AWDL `-pion-net -candidate-policy "$CANDIDATE_POLICY"` WebRTC plus Network.framework UDP perf in both directions, simultaneous bidirectional UDP perf, latency, and callback probes. After each listener prints an address, the matrix captures listener-side route, `lsof`, and `netstat` output for that UDP port plus sender-side route checks before sending. After setup, per-profile probes continue after failures and end with a matrix summary. `scripts/remote-diagnostics.sh` captures deeper route/interface state from both hosts; `NW_CONNECT_TIMEOUT` and `NW_CONNECT_RETRIES` tune Network.framework readiness retry for every local and remote demo command. Currently blocked because SSH to `tmc2@10.0.18.249` times out. |
| Remote matrix summary | `scripts/remote-matrix-bundle.sh` or `go run ./cmd/matrix-summary /tmp/awdl-webrtc-matrix.txt > /tmp/awdl-webrtc-matrix-summary.md` | Produces the clean Markdown result table after the matrix has emitted JSON records or `FAIL:` lines. The bundle writes the summary even when the matrix fails, then returns the matrix exit code. On the current SSH-blocked transcript it emits a `remote reachability` failure row, which keeps setup failures visible in the at-a-glance table. |
...
| AWDL WebRTC needs explicit candidate handling | Pion mDNS did not resolve into an open AWDL datachannel here; `-mdns disabled -candidate-policy auto` uses explicit link-local candidate signaling to open the AWDL `SetNet` datachannel without a Pion fork. |
