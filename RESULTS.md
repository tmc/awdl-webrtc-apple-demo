...
| WebRTC support | PASS | Yes, via Pion WebRTC with constrained ICE. | Modes: `check`, `gather`, `pair`, `answer-stdio`, `offer-stdio`, `offer-ssh`. | `offer-stdio` makes the wire signal independent of SSH; it is still manual signaling, not automatic discovery. |
...
| Non-SSH WebRTC signaling | PARTIAL | Manual stdio signaling is implemented for two-host WebRTC tests when SSH is blocked. | `offer-stdio` prints an `OFFER` line and waits for an `ANSWER` line from `answer-stdio`; both use the same explicit candidate wire format as `offer-ssh`; parser tests pass. | This removes SSH from signaling, but still needs a two-host run. A same-host `en0` smoke exchanged the signal lines but ICE did not open the datachannel. |
