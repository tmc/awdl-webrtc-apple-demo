...
[docs/matrix-20260519-success.md](docs/matrix-20260519-success.md).

| Area | Current result |
| --- | --- |
| Published backend pins | `github.com/tmc/apple v0.6.7`; `github.com/tmc/apple-pion v0.1.3`; no local replace. |
...
| AWDL | Pion-native WebRTC PASS; Network.framework UDP perf/latency/callback probes pass both directions over `awdl0/NWInterfaceTypeWifi`. |
| Matrix artifacts | Raw transcript `/tmp/awdl-webrtc-matrix-signalonly-followup.txt`; generated table `/tmp/awdl-webrtc-matrix-signalonly-followup.md`; checked-in table [docs/matrix-20260519-success.md](docs/matrix-20260519-success.md). |
...
| Pion `transport.Net` package | PASS | `github.com/tmc/apple-pion/nwtransport` implements the Pion `transport.Net` UDP surface needed by ICE, and the demo imports published `v0.1.3`. | `v0.1.1` filters the selected interface to the configured local address; `v0.1.2` moves the link-local candidate policy to Pion address rewrite rules; current LAN, Thunderbolt, and AWDL `-pion-net -mdns disabled -candidate-policy auto offer-ssh` opened datachannels and exchanged `ping`/`pong`. | DNS, TCP, unconstrained wildcard UDP, TURN/STUN helper traffic outside the selected UDP surface, and unsupported UDP cases intentionally fall back to Pion's standard network. |
...
| Network.framework remote UDP | PASS | Multi-datagram echo, latency, and callback probes work across LAN, Thunderbolt, and AWDL in both directions. | Current matrix records Network.framework path evidence for `en0`, `bridge0`, and `awdl0`; callback probes passed both directions on all three profiles. | One LAN local-to-remote aggregate saw 5% loss; longer sweeps are still needed. |
...
| AWDL remote UDP perf | PASS | Two-host AWDL UDP completed in both sequential and bidirectional directions. | Current matrix: remote-to-local 60/60 datagrams, `8.71 Mbits/sec`; local-to-remote 60/60 datagrams, `8.12 Mbits/sec`; both report path `awdl0/NWInterfaceTypeWifi`. | Longer duration and multi-stream sweeps are still needed for stable throughput claims. |
| AWDL Network.framework UDP | PASS | Network.framework sends raw UDP perf, latency, and callback traffic over AWDL in both directions. | Current matrix: latency summaries average `6.271ms` and `5.968ms`; callback probes returned `callback:callback` both directions. | Short-run RTT has occasional outliers. |
...
| Thunderbolt remote UDP perf | PASS | Two-host Thunderbolt UDP completed in both sequential and bidirectional directions. | Current matrix: remote-to-local 60/60 datagrams, `29.35 Mbits/sec`; local-to-remote 60/60 datagrams, `39.22 Mbits/sec`; both report path `bridge0/NWInterfaceTypeWired`. | Required interface type is omitted; the link-local bridge address selects `bridge0`. |
| Thunderbolt Network.framework UDP | PASS | Network.framework sends raw UDP perf, latency, and callback traffic over Thunderbolt Bridge in both directions. | Current matrix: latency summaries average `821.232us` and `651.397us`; callback probes returned `callback:callback` both directions. | Short-run matrix numbers include occasional high RTT outliers. |
...
| LAN Network.framework UDP | PASS | Network.framework sends raw UDP perf, latency, and callback traffic over LAN in both directions. | Current matrix: remote-to-local 60/60 datagrams, `6.98 Mbits/sec`; local-to-remote 57/60 datagrams, `979.39 Kbits/sec`; callback probes returned `callback:callback` both directions. | LAN remains slower/less stable than Thunderbolt in this environment. |
...
| Remote SSH | PASS for the current run: `ssh -o BatchMode=yes -o ConnectTimeout=5 tmc2@10.0.18.249 'echo ok'` returned `ok` before the matrix, and no SSH command failed during the successful run. Earlier 2026-05-19 attempts are preserved in [docs/matrix-try-again-20260519.md](docs/matrix-try-again-20260519.md). |
| Remote matrix success | `WEBRTC_TRACE=1 REMOTE_READY_TIMEOUT=30 REMOTE_STEP_READY_TIMEOUT=30 CANDIDATE_POLICY=auto REQUIRE_PATHS=1 SSH_TARGET=tmc2@10.0.18.249 REMOTE_BIN=/Volumes/Shared/awdl-webrtc-apple-demo-bin OUTPUT=/tmp/awdl-webrtc-matrix-signalonly-followup.txt SUMMARY_OUTPUT=/tmp/awdl-webrtc-matrix-signalonly-followup.md scripts/remote-matrix-bundle.sh` wrote [docs/matrix-20260519-success.md](docs/matrix-20260519-success.md). LAN, Thunderbolt, and AWDL Pion-native WebRTC passed, and UDP perf, latency, and callback probes passed with path evidence. |
...
| Network backend build pin | `go list -m -f '{{.Path}} {{.Version}} {{.Replace.Path}}' github.com/tmc/apple github.com/tmc/apple-pion` | `github.com/tmc/apple v0.6.7`; `github.com/tmc/apple-pion v0.1.3`. |
...
| Remote productization matrix | `WEBRTC_TRACE=1 REMOTE_READY_TIMEOUT=30 REMOTE_STEP_READY_TIMEOUT=30 CANDIDATE_POLICY=auto REQUIRE_PATHS=1 SSH_TARGET=tmc2@10.0.18.249 REMOTE_BIN=/Volumes/Shared/awdl-webrtc-apple-demo-bin OUTPUT=/tmp/awdl-webrtc-matrix-signalonly-followup.txt SUMMARY_OUTPUT=/tmp/awdl-webrtc-matrix-signalonly-followup.md scripts/remote-matrix-bundle.sh` | Runs LAN/Thunderbolt/AWDL `-pion-net -candidate-policy auto` WebRTC plus Network.framework UDP perf in both directions, simultaneous bidirectional UDP perf, latency, and callback probes. Current result: all three WebRTC legs pass; UDP probes pass with path evidence. |
...
| Pion-native LAN pair | `go run . -profile lan -backend network -pion-net -mode pair -timeout 15s`; `GOWORK=off go run . -profile lan -backend network -pion-net -mdns disabled -candidate-policy raw -mode pair -timeout 15s` | Current same-host checks timed out before datachannel open. With mDNS, no remote candidates resolved; with raw candidates, both sides installed `10.0.199.147` host candidates but candidate-pair checks saw requests and zero responses. The recorded two-host LAN `offer-ssh` matrix remains the stronger LAN Pion-native evidence. |
...
| Network LAN reverse perf | Remote listener plus local sender to `10.0.18.249` | Current matrix: 57/60 datagrams, 5% loss, `979.39 Kbits/sec`, avg RTT `9.373ms`. |
| Network Thunderbolt reverse perf | Remote listener plus local sender to `169.254.88.35` | Current matrix: 60/60 datagrams, zero loss, `39.22 Mbits/sec`, avg RTT `1.044ms`. |
| Network AWDL reverse perf | Remote listener plus local sender to `[fe80::5c89:22ff:fe01:380d%awdl0]` | Current matrix: 60/60 datagrams, zero loss, `8.12 Mbits/sec`, avg RTT `8.422ms`. |
...
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| LAN `en0` | `network` | `en0/NWInterfaceTypeWifi` | 140.62 KiB | 6.98 Mbits/sec | 60 | 4.240ms | 10.517ms | 9.141ms | 25.540ms | 25.589ms |
| Thunderbolt `bridge0` | `network` | `bridge0/NWInterfaceTypeWired` | 140.62 KiB | 29.35 Mbits/sec | 60 | 112.667us | 2.376ms | 255.667us | 32.040ms | 32.058ms |
| AWDL `awdl0` | `network` | `awdl0/NWInterfaceTypeWifi` | 140.62 KiB | 8.71 Mbits/sec | 60 | 3.931ms | 8.358ms | 7.085ms | 26.404ms | 26.456ms |
...
| --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| LAN `en0` | `network` | `10.0.18.249` | PARTIAL | 133.59 KiB | 979.39 Kbits/sec | 57 | 3 | 5.00% | 5.362ms | 9.373ms | 8.460ms | 23.122ms | 23.176ms |
| Thunderbolt `bridge0` | `network` | `169.254.88.35` | PASS | 140.62 KiB | 39.22 Mbits/sec | 60 | 0 | 0.00% | 237.542us | 1.044ms | 566.333us | 7.558ms | 7.976ms |
| AWDL `awdl0` | `network` | `[fe80::5c89:22ff:fe01:380d%awdl0]` | PASS | 140.62 KiB | 8.12 Mbits/sec | 60 | 0 | 0.00% | 4.712ms | 8.422ms | 7.034ms | 24.196ms | 24.257ms |
...
| UDP direction is no longer the main blocker | The current matrix passes sequential Network.framework UDP perf, latency, and callback probes in both directions for LAN, Thunderbolt, and AWDL. One LAN local-to-remote aggregate saw 5% loss. |
