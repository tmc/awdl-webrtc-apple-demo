...
| Matrix reachability diagnostics | `CONNECT_TIMEOUT=1 PROFILES=lan CANDIDATE_POLICY=auto OUTPUT=/tmp/awdl-webrtc-matrix-listener-diag-smoke.txt scripts/remote-matrix.sh` | Before the SSH timeout, the matrix transcript captured `candidate_policy=auto`, route-to-peer via gateway `172.20.10.1` on `en0`, 100% ping loss, TCP/22 timeout, `scutil --nwi`, and local interface inventory. |
...
| Remote matrix summary | `go run ./cmd/matrix-summary /tmp/awdl-webrtc-matrix.txt > /tmp/awdl-webrtc-matrix-summary.md` | Produces the clean Markdown result table after the matrix has emitted JSON records or `FAIL:` lines. On the current SSH-blocked transcript it emits only the table header, which correctly shows no perf records or matrix-step failures were reached. |
