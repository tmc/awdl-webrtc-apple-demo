...
| SwiftUI link monitor | PARTIAL: [docs/ui-two-host.md](docs/ui-two-host.md) records a readable local UI active-path run fed by a remote headless discovery publisher. It showed Thunderbolt selected, LAN/Thunderbolt/AWDL possible paths, and bandwidth history after the text-layout fix. [scripts/run-ui-harness.sh](scripts/run-ui-harness.sh) now starts the remote publisher, runs local discovery preflight, and opens the local UI for the physical proof. It builds through the checked-in workspace by default and accepts `BUILD_GOWORK=off` for the current published-module fallback. Its `LAUNCH_UI=0` smoke passed from clean commit `d7da303`, and `LAUNCH_UI=0 BUILD_GOWORK=off` passed from clean commit `1288332`, discovering the current remote publisher with LAN, Thunderbolt, and AWDL ready. Physical Thunderbolt-removal fallback is still not visually proven. |
...
| UI fallback selection | PARTIAL | The link-health sampler tries Thunderbolt, then AWDL, then LAN. The rendered monitor now keeps rows readable instead of collapsing text, and `scripts/run-ui-harness.sh` provides a repeatable two-Mac launch path for the manual proof. | `TestLinkHealthSamplePreferredFallsBackToAWDL` and `TestLinkHealthSamplePreferredSkipsUnavailableThunderbolt` pass; two-host headless discovery found live LAN, Thunderbolt, and AWDL paths; a local visible UI run with a remote headless publisher showed Thunderbolt selected and bandwidth history updating; the harness syntax-checks, builds through the normal Go binary by default, can build with `BUILD_GOWORK=off`, and both non-visual smokes passed against `tmc2@10.0.18.249` with all three link rows ready. | The SwiftUI window still needs a two-live-Mac visual run and physical Thunderbolt removal observation. |
...
| Local SwiftUI active-path view | Local `go run . -mode ui -backend network -ui-interval 2s -ui-count 20 -ui-window 4`; remote `tmc2@10.0.18.249` headless `-mode discover -backend network -timeout 20m -ui-interval 2s` | The rendered UI showed active path `thunderbolt`, peer `MacBook-m4small.local`, possible paths for Thunderbolt/AWDL/LAN, and bandwidth history rows around 18-25 Mbit/s with 0.0% loss. This verifies the text-layout fix and active-path visualization. |
