...
| Network.framework readiness retry | PASS | The demo uses `nwpacket.Config.ConnectTimeout=2s` and `ConnectRetries=2` for Network.framework UDP and Pion `transport.Net` PacketConns. | `tmc/apple v0.6.7` adds outbound peer eviction/recreate after readiness timeout; `go test ./x/network/nwpacket`; demo `go test ./...`. | Two-host rerun is still needed to see whether this clears the old local-to-remote AWDL/Thunderbolt readiness asymmetry. |
